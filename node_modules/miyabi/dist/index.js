...
import { createOmegaCommand } from './commands/omega.js';
import { registerPipelineCommand } from './commands/pipeline.js';
import { loadConfig, applyConfigToEnvironment } from './config/loader.js';
import { reportIssueToMiyabi, gatherEnvironmentInfo, gatherProjectContext, inferUserIntent, } from './feedback/issue-reporter.js';
...
program
    ...
    if (!isInteractiveTerminal()) {
        // AI agent向けJSON出力
        if (isJsonMode()) {
            outputError('INVALID_COMMAND_OR_NON_INTERACTIVE', 'Interactive mode not available in non-interactive terminal', true, 'Specify a command directly: miyabi status --json');
            process.exit(1);
        }
 ...
 */
async function handleErrorAndReport(action, error) {
    try {
        // Gather context
        const context = {
