...
export async function setup(options = {}) {
    ...
    // Step 1: Welcome
    const readyToStart = await confirmOrDefault('セットアップを開始しますか？', true, {
    ...
    const hasGitHubAccount = await confirmOrDefault('GitHubアカウントをお持ちですか？', true, {
        nonInteractive: options.nonInteractive,
        yes: options.yes,
    });
    if (!hasGitHubAccount) {
        if (nonInteractiveMode) {
            ...
            const shouldOpenBrowser = await confirmOrDefault('ブラウザで https://github.com を開きますか？', true, {
                nonInteractive: options.nonInteractive,
                yes: options.yes,
            });
            ...
            const accountCreated = await confirmOrDefault('アカウントは作成できましたか？', false, {
                nonInteractive: options.nonInteractive,
                yes: options.yes,
            });
            if (!accountCreated) {
    ...
    switch (nextAction) {
        ...
        case 'guide':
            ...
            if (openGuide) {
                await open('https://github.com/ShunsukeHayashi/Autonomous-Operations/blob/main/packages/cli/SETUP_GUIDE.md');
            }
            break;
        case 'exit':
            console.log(chalk.cyan('\n👋 Miyabiへようこそ！\n'));
