...
import { isJsonMode, outputSuccess, outputError, logVerbose, getGitHubToken } from '../utils/agent-output.js';
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('404') || error.message.includes('Not Found')) {
                outputError('REPOSITORY_NOT_FOUND', 'Repository not found or no access', true, 'Check repository permissions and GITHUB_TOKEN scope');
            }
            if (error.message.includes('401') || error.message.includes('Unauthorized')) {
                outputError('AUTH_INVALID_TOKEN', 'Authentication failed - token is invalid', true, 'Generate a new GITHUB_TOKEN: https://github.com/settings/tokens');
            }
            if (error.message.includes('403') || error.message.includes('Forbidden')) {
                outputError('AUTH_INSUFFICIENT_PERMISSIONS', 'Access denied - insufficient permissions', true, 'Ensure GITHUB_TOKEN has repo scope');
            }
            outputError('NETWORK_ERROR', `Failed to fetch status: ${error.message}`, true, 'Check network connection and GitHub API status');
        }
        throw error;
    }
}
...
async function getStatusData(octokit, owner, repo) {
    ...
    for (const issue of issues) {
        for (const label of issue.labels) {
            ...
            if (labelName.includes('state:pending'))
                states.pending++;
            else if (labelName.includes('state:analyzing'))
                states.analyzing++;
            else if (labelName.includes('state:implementing'))
                states.implementing++;
            else if (labelName.includes('state:reviewing'))
                states.reviewing++;
            else if (labelName.includes('state:blocked'))
                states.blocked++;
            else if (labelName.includes('state:paused'))
                states.paused++;
        }
    }
    // Fetch recent PRs
...
function displayStatusHuman(data) {
