...
async function refreshDashboard(options) {
    const { url = DEFAULT_SERVER_URL, target = 'all', json = false, watch = false, } = options;
 ...
 */
async function performRefresh(url, target, json, timeout) {
    ...
    try {
        ...
        if (json) {
            ...
            if (spinner)
                spinner.succeed('Dashboard refreshed');
        ...
        if (json) {
            ...
            if (spinner)
                spinner.fail('Failed to refresh dashboard');
 ...
 */
async function dashboardStatus(options) {
    ...
    try {
        ...
        if (json) {
            ...
            if (devicesData.devices) {
                const online = devicesData.devices.filter((d) => d.status === 'online').length;
                const idle = devicesData.devices.filter((d) => d.status === 'idle').length;
