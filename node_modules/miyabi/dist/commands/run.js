...
import { createDashboardState, renderDashboard, clearScreen, addAgent, updateAgentStatus, addLog, calculateOverallProgress, getAgentDisplayName, } from '../utils/tui-dashboard.js';
import { HumanInTheLoop, } from '../utils/human-in-the-loop.js';
/**
 ...
 */
async function simulateAgentExecution(agentName, state, hitl, options) {
    ...
    if (['deploy', 'pr'].includes(agentName)) {
        const gate = HumanInTheLoop.createGate(agentName === 'deploy' ? 'deploy' : 'merge', { agent: agentName, issue: options.issue });
...
async function selectTaskInteractively() {
    ...
    return { taskType: taskType, issue };
}
/**
 * Run the execution with TUI dashboard
 */
async function runWithDashboard(taskType, options) {
