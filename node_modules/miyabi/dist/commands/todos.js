 ...
 * TODOをIssueに変換
 */
async function convertToIssue(todo, options) {
    if (options.dryRun) {
 ...
 * Todos Auto Mode実行
 */
export async function runTodosAutoMode(options) {
    ...
    // 1. TODOスキャン
    const todos = scanDirectory(targetPath);
    spinner.succeed(chalk.green(`${todos.length}件のTODOコメントを検出`));
    if (todos.length === 0) {
    ...
    sortedTodos.slice(0, 20).forEach((todo, index) => {
        const priorityColor = todo.priority === 1
            ? chalk.red
            : todo.priority === 2
                ? chalk.yellow
                : chalk.gray;
    ...
    if (options.createIssues) {
        ...
        console.log(chalk.yellow('💡 Issue作成: --create-issues オプションを使用してください\n'));
    }
}
/**
