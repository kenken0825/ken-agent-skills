...
import { IssueAgent, CodeGenAgent, ReviewAgent, CoordinatorAgent, } from 'miyabi-agent-sdk';
import { PRAgent, } from 'miyabi-agent-sdk';
import { getGitHubToken, verifyTokenAccess, } from '../auth/credentials.js';
import { saveCodeGenOutput, loadCodeGenOutput, saveReviewOutput, loadReviewOutput, checkIssueStorage, } from '../utils/storage.js';
/**
 * 利用可能なAgent種別
 */
 ...
 */
export function parseIssueNumber(issueInput) {
 ...
 * Agent実行
 */
export async function runAgent(agentName, options) {
    ...
    try {
        ...
        if (!options.verbose)
            spinner.text = 'Verifying repository access...';
        const hasAccess = await verifyTokenAccess(githubToken, owner, repo);
        if (!hasAccess) {
            spinner.fail(chalk.red('Access denied'));
            throw new Error(`You don't have access to ${owner}/${repo}. ` +
                'Make sure you have the correct permissions.');
        }
        if (!options.verbose)
            spinner.text = `${agentName}Agent 実行中...`;
        ...
        switch (agentName) {
            ...
            case 'pr': {
                ...
                if (!storage.hasCodeGen) {
                    throw new Error(`CodeGenAgent output not found for issue #${issueNumber}. ` +
                        'Please run: miyabi agent run codegen --issue=' + issueNumber);
                }
                if (!storage.hasReview) {
                    throw new Error(`ReviewAgent output not found for issue #${issueNumber}. ` +
 ...
 * Agent状態確認
 */
export async function agentStatus(agentName) {
    console.log(chalk.cyan.bold('\n📊 Agent実行状態\n'));
    const table = new Table({
        head: [
            chalk.white('Agent'),
...
export function registerAgentCommand(program) {
    ...
    agent
        ...
        if (!AVAILABLE_AGENTS.includes(agentName)) {
            // AI agent向けJSON出力
            if (isJsonMode() || options.json) {
                outputError('INVALID_AGENT_NAME', `Invalid agent: ${agentName}`, true, `Available agents: ${AVAILABLE_AGENTS.join(', ')}`);
            }
        ...
        if (isJsonMode() || options.json) {
            if (result.status === 'success') {
                ...
                outputError('AGENT_EXECUTION_FAILED', result.message, true, 'Check agent logs for details', result);
            }
            return;
        }
    ...
    agent
        ...
        .option('--json', 'JSON形式で出力')
        .action(async (agentName, _options) => {
        // TODO: Implement JSON output
        await agentStatus(agentName);
    });
}
