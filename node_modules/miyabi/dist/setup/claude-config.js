...
import { validateDeployClaudeConfigParams, validateDeployToGitHubParams, validateProjectPath, validateFilePath, validateFileSize, validateFileContent, validateSymlink, sanitizeTemplateVariable, } from './validation.js';
 ...
 */
export async function deployClaudeConfig(options) {
 ...
 */
export async function deployClaudeConfigToGitHub(owner, repo, projectName, token) {
    ...
    const blobs = await Promise.all(filesToCommit.map(async (file) => {
 ...
 */
async function collectDirectoryFiles(srcDir, basePrefix) {
    const files = [];
 ...
 */
export async function verifyClaudeConfig(projectPath) {
